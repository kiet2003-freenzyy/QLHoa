...
<head>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $trangthai_hd = $_POST['trangthai_hd'];
        $feedback = $_POST['feedback'];
        $ma_nv = $_POST['ma_nv'];

        $ngay_dat = $_POST['ngay_dat'];
        $ngay_giao = $_POST['ngay_giao'];
        $tien_datcoc = $_POST['tien_datcoc'];
        $ma_giamgia = $_POST['ma_giamgia'];
        ...
        $sql = "UPDATE hoa_don SET ma_kh = ?, ten_nguoinhan = ?, diachi_nguoinhan = ?, sdt_nguoinhan = ?, tong_tien = ?, trangthai_hd = ?, feedback = ?, ma_nv = ? WHERE ma_hd = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$ma_kh, $ten_nguoinhan, $diachi_nguoinhan, $sdt_nguoinhan, $tong_tien, $trangthai_hd, $feedback, $ma_nv, $ma_hd1]);

        // Cập nhật bảng chitiet_hoadon
        $sql = "UPDATE chitiet_hoadon SET ngay_dat = ?, ngay_giao = ?, tong_tien = ?, tien_datcoc = ?, ma_giamgia = ?, hinhthuc_tt = ?, ghi_chu = ? WHERE ma_hd = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$ngay_dat, $ngay_giao, $tong_tien, $tien_datcoc, $ma_giamgia, $hinhthuc_tt, $ghi_chu, $ma_hd1]);

        // Cập nhật bảng hoadon_hoa
        foreach ($_POST['ma_hoa'] as $index => $ma_hoa) {
            $so_luong = $_POST['so_luong'][$index];
            $sql = "UPDATE hoadon_hoa SET ma_hoa = ?, so_luong = ? WHERE ma_hd = ? AND ma_hoa = ?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$ma_hoa, $so_luong, $ma_hd1, $ma_hoa]);
        }
        header("Location: edit_hoadon.php?ma_hd=".$ma_hd1);
    }
